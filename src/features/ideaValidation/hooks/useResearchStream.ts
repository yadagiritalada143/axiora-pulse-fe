...
export function extractFromValidationResult(result: OrchestrationResult | null | undefined): {
  queries: ResearchStreamQueryEvent[];
  sources: ResearchStreamSourceEvent[];
} {
  if (!result) return { queries: [], sources: [] };
  ...
  return { queries: realQueries, sources: realSources };
}
...
export function useResearchStream({
  ...
  const [streamQueries, setStreamQueries] = useState<ResearchStreamQueryEvent[]>([]);
  const [streamSources, setStreamSources] = useState<ResearchStreamSourceEvent[]>([]);
  ...
  const extracted = useMemo(() => extractFromValidationResult(result), [result]);

  const queries = streamQueries.length > 0 ? streamQueries : extracted.queries;
  const sources = streamSources.length > 0 ? streamSources : extracted.sources;
        ...
        try {
          ...
          if (eventName === 'snapshot' && parsed.data) {
            ...
            if (Array.isArray(snap.queries)) {
              setStreamQueries(snap.queries);
            }
            if (Array.isArray(snap.sources)) {
              setStreamSources(snap.sources);
            }
  ...
  return {
    ...
    activeQuery,
    isStreaming,
    isComplete: isComplete || (result !== null && result !== undefined),
    error,
