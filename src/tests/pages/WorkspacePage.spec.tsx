...
jest.mock('@features/workspace/components', () => ({
  CreateWorkspaceDialog: function CreateWorkspaceDialog({ open }: { open: boolean }) {
    return <div data-testid="create-dialog">{open ? 'create-open' : 'create-closed'}</div>;
  },
  WorkspaceEmpty: function WorkspaceEmpty({ onCreate }: { onCreate: () => void }) {
    return <button onClick={onCreate}>empty-create</button>;
  },
  WorkspaceGrid: function WorkspaceGrid({
    workspaces,
    onWorkspaceClick,
    onWorkspaceDelete,
  }: {
  ...
  DeleteWorkspaceDialog: function DeleteWorkspaceDialog({
    open,
    loading,
    workspaceName,
    onConfirm,
